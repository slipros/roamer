...
// BenchmarkSetString_WithCachedTypeVsDirect compares using cached type vs direct type check.
// This is to demonstrate the performance benefit of using type caching.
func BenchmarkSetString_WithCachedTypeVsDirect(b *testing.B) {
	// Function that uses cached type (how it's implemented now)
	useCachedType := func(field reflect.Value, str string) error {
	...
	// Function that calculates type each time (original implementation)
	useDirectType := func(field reflect.Value, str string) error {
	...
	// Create a time.Time field for testing
	var t time.Time
	field := reflect.ValueOf(&t).Elem()
	timeStr := "2023-01-15T12:00:00Z"

	// Benchmark with cached type
	b.Run("CachedType", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_ = useCachedType(field, timeStr)
		}
	})

	// Benchmark with direct type check
	b.Run("DirectType", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_ = useDirectType(field, timeStr)
		}
	})
}
